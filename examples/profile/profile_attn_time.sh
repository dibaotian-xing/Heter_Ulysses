...
function profile_attn_time(){
    for (( i=0 ; i<${#gpu_type_id[@]} ; i=i+1 ))
    do
      bash $SCRIPT_PATH ${gpu_type_id[i]} ${gpu_type_rank[i]} $hidden_size $num_attention_heads $num_query_groups
      ...
      bash $SCRIPT_PATH ${gpu_type_id[i]} ${gpu_type_rank[i]} $hidden_size_more $num_attn_heads_more $num_gqa_groups_more 
    done
}